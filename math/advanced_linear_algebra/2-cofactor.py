...
def cofactor(matrix):
    ...
    The cofactor matrix is a matrix where each element (i,j) is the cofactor
    of the element at position (i,j) in the original matrix. The cofactor is
    calculated as (-1)^(i+j) * minor(i,j), where minor(i,j) is the determinant
    of the submatrix obtained by removing row i and column j.
